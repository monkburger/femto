...
func writeTOML(t *testing.T, content string) string {
	t.Helper()
	f, err := os.CreateTemp(t.TempDir(), "femto-*.toml")
	if err != nil {
		t.Fatal(err)
	}
...
func TestListenAddrResolve(t *testing.T) {
	cases := []struct {
		...
		// errors
		{"", "", true},
		{"0", "0.0.0.0:0", false},           // bare port 0: Resolve succeeds; validate() rejects later
		{"abc", "", true},                   // no port
...
func TestLoadMissingFile(t *testing.T) {
	_, err := Load("/nonexistent/femto.toml")
	if err == nil {
...
cache_max_age = "7d"
  ...
  key  = "` + keyFile + `"
`
	// 7d is not a standard Go duration — use a standard one.
	tomlStr = `
[[vhost]]
server_names  = ["x.com"]
document_root = "` + dir + `"
