...
// ListenAddr is a single listener specification from the config file.
// It can be written as any of:
//
//	"443"              – port only, binds to 0.0.0.0 (all IPv4)
//	"*:443"            – all IPv4 interfaces, port 443
//	"[::]:443"         – all IPv6 interfaces, port 443
//	"127.0.0.1:8443"   – specific IPv4 address
//	"[::1]:8443"       – specific IPv6 address (bracketed)
...
// Duration wraps time.Duration so it can be parsed directly from a TOML string
// using Go's standard duration syntax (e.g. "5s", "1m30s").
...
// When enabled, femto starts a plain-HTTP (no TLS) server on the socket in
...
// femto over the socket.
type UnixConfig struct {
	Enabled bool   `toml:"enabled"`
	Path    string `toml:"path"` // e.g. "/run/femto/femto.sock"
...
// When enabled, Femto binds one or more plain-HTTP ports and responds to every
...
// Config is the top-level structure parsed from femto.toml.
type Config struct {
