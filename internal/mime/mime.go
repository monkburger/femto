...
//  3. Custom file - the path configured in femto.toml [server] mime_types.
package mime
...
func Init(customPath string) error {
	...
	if customPath != "" {
		...
		log.Printf("femto: mime: loaded custom types from %s", customPath)
	}
	return nil
}
...
func loadSystemFiles() {
	for _, p := range systemPaths {
		...
		if err := loadFile(p); err != nil {
			log.Printf("femto: mime: skipping %s: %v", p, err)
			continue
		}
		log.Printf("femto: mime: loaded system types from %s", p)
	}
}

// loadFile parses a mime.types file and registers each entry.
// Format: <mime-type> <ext> [<ext> ...] with '#' comment lines.
// Also handles the BSD-reversed format: <ext> [<ext> ...] <mime-type>.
